...
public class ImageUploadController {
    ...
    @PostMapping("/upload")
    public ResponseEntity<MessageRespone> uploadImageToUser(@RequestParam("file") MultipartFile file,
                                                      Principal principal) throws IOException {
        imageUploadService.uploadImageToUser(file,principal);
        return ResponseEntity.ok(new MessageRespone("Image uploaded successfully"));
    }
    @PostMapping("/{postId}/upload")
    public ResponseEntity<MessageRespone> uploadImageToPost(@PathVariable("postId") String postId,
